...
class LexiCWI(LexiPersonalizedPipelineStep):
    ...
    def is_complex(self, sent, startOffset, endOffset):
        cwi_score = self.scorer.score(sent, startOffset, endOffset) + 0.04
...
class SimpleLexiScorer:

    def __init__(self, userId, featurizer):
        self.userId = userId
        self.featurizer = featurizer
        self.path = SIMPLE_SCORER_PATH_TEMPLATE.format(userId)
    ...
    def score(self, sent, start_offset, end_offset):
        score = end_offset - start_offset  # only length
        return score
